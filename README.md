...
A application neutral delivery model should be based on declarative definitions that trigger system changes matching the topology and the lifecycle of an application automatically, without prescribing specific communication patterns, packaging mechanisms or orchestration services. System configuration and provisioning scripts should not prevent engineers from managing dependencies on operating system level or orchestrating services on mutable hosts. While the development of cloud controllers is derived from the idea that every system can be virtualized, evolving technologies raise the question, whether this is the only way to deliver programmable infrastructure. A package manager that interprets declarative templates enables engineers to provide purpose build operating systems. And distributing configuratons via git allows to centralize management tasks, to track and roll back system configurations. The nix package manager enables engineers to compose operating systems declaratively, without abstracting the runtime environment, network and storage interfaces. The strip down version of an the operating system that includes only the most essential components for basic functionality is extended with two main components:
