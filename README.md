...
Developing services for a hybrid cloud can be difficult, a large number interdependencies, complex network configurayions and granular security settings  make it tough to replicate backing services outside the dynamic part of the cloud. This project offers a configurable sandbox to speed up development by solving the replication problem with a maschine centered setup. It creates a reproducible copy of all service components without predefined networks and without orchestrator dependencies. The sandbox is isolated at the operating system level. Service components are kept separate and won't interfere with the host system. It is built on NixOS and uses the Nix package manager to guarantee that the environment is reproducible. With some tweaks, it can also run on [Windows (via WSL)](https://learn.microsoft.com/en-us/windows/wsl/about), [ChromeOS (via CROSH)](https://chromeos.dev/en/linux), and [macOS](https://github.com/LnL7/nix-darwin).
