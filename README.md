# Developer Sandbox
Developing hybrid cloud services can be challenging because replicating productive services is not always possible. But mismatches in network and service configurations lead to extensive regression tests that delay of go-lives. This repository aims to accelerate service developments with an opinionated NixOS setup that empowers platform enigneers with configurable sandbox environments. Sandboxes represent predfined service compositions but remain flexibile to add artifacts like applications, runtimes or services. Components can be hosted services, operating system extensions or packaged applications, as binaries and/or containers. The sandbox is built on NixOS and utilizes the Nix package manager to ensure robust reproducibility. However, with a few tweaks it can also run on [Windows (via WSL)](https://learn.microsoft.com/en-us/windows/wsl/about), [ChromeOS (via CROSH)](https://chromeos.dev/en/linux), and [macOS](https://github.com/LnL7/nix-darwin).
