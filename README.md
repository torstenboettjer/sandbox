...
The operator sandbox provides an development and execution environment for operation engineers, migrating applications to a hybrid cloud stack. It introduces an approach towards "infrastructure as coden (IaC)" that meets enterprise requirements. The focus is on automating management processes in service operation, and retaining control over application deployments and service provisioning. Usually, cloud engineers employ a combination of applications like Terraform, Ansible and Kubernetes to automate system configurations, topology designs and container deployments. Yet configuration management systems, provisioning tools and container platforms serve a similar purpose and overlap significantly in functionality. The focus is merely on deploying services on managed infrastructure. While this might be applicable for cloud-native applications, it rises complexity, operatoring large quantities of applications sourced from different vendors and it makes it hard to retain control over data and address technical, commercial or regulatory requirements. IaC tools combine system definitions and execution instructions in a common code base, while ensuring regulatory compliance, testing security policies and providing commercial justifications before launching a service requires a separation. Continuous integrations and deployments for enterprise services requires an integrated toolchain rather than a single application. And process automation demands for a common data model rather than a single code base. 
