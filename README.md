...
The operator sandbox provides an development and execution environment for operation engineers, migrating applications to a hybrid cloud. It introduces an approach towards "infrastructure as code (IaC)" that meets enterprise requirements. The focus is on automating management processes, retaining control over application deployments and centralize service provisioning. Usually, cloud engineers employ a combination of applications like Terraform, Ansible and Kubernetes to automate system configurations, topology designs and container deployments. Yet configuration management systems, provisioning tools and container platforms overlap significantly in functionality. These tools serve a similar purpose, the focus is on deploying- rather than on managing services. The implications on the application design are best described in the [Twelve-Factor](https://12factor.net/) manifesto. While this is benefitial for operators of cloud-native applications, it rises complexity for operators managing a portfolio of third party applications. The lack of control makes it difficult to ensure the integrity of business-critical data and to meet technical, commercial, or regulatory requirements with appropriate operation procedures. Therefor, a hybrid platform uses public cloud infrastructure only where appropriate. The delivery model distinguishes between dedicated- and distributed systems where controllers are either provided by the physical resource or exposed through an orchestrator. While cloud provider enable infrastructure programmability through an orchestrator, private infrastructure relies on physical controller that expose interfaces on operating system level. The focus is on IaC, because common tools combine system definitions and execution instructions in a common code base, while ensuring regulatory compliance, testing security policies or providing commercial justifications need to take place before launching a service and the code that defines services has to be separated from the the code that launches services. 

## Base System

Declarative package managers like [Guix](https://guix.gnu.org/) or [Nix](https://github.com/NixOS/nix) provide programmability on operating system level. Operators define desired system states in configuration files that isolate the dependencies for software packages and ensure clean and reproducible systems, including the user- and the kernel-space. System administrators to write templates for purpose build systems in a functional programming language. The golden image is a strip down version of the linux operating system that only covers the most essential components for basic functionality. System templates trigger changes to the operating system that match runtime requirements and the topology design without depending on specific communication patterns, packaging mechanisms or orchestrator capabilities. This enables operation teams to centralize management tasks, to track and to roll back system configurations in a similar way like immutable artifacts but without abstraction of the runtime environment, network- and storage interfaces.
