...
The engineering sandbox provides an environment for service developers to develop integrations between business applications on a local maschine. It is centered around solution composositions that enable selected business processes. It builds on the nix package and captures deployment functions in the nix language to ease the transition between the development-, test- to production stage. A declarative package manager makes deployment tools like Terraform, Ansible and Kubernetes obsolete and leaves the decision for a specific operating mode to the system administrators. While common configuration infrastructure-as-code and platform-automation tools combine application requirements, system definitions and implementation instructions in a single code-base, we separate the application requirements from system and cloud provider dependencies to ingest security and compliance controls on platform level, so that application deployments can be tested against regulatory compliance, security policies before a service is launched.
