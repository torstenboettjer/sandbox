...
The definition of the host system separated from the home configuration and stored in the [flake.nix](./flake.nix) file. This allows to run the same toolset across developers independent from from any preference for the one or other operating system or hardware platform. Relying on a separate configuration file for system and platform definitions eases continuous deployments together with the application code, while **[direnv](https://direnv.net/)** enables system engineers to support multiple projects with purpose build systems. It is a shell extension to load and unload system software and configurations automatically, moving in and out a directory. direnv isolates system configurations their dependencies in a directory and automatically loads and unloads the components, switching from one directory to another. This helps to overcome one of the complexity driver for DevOps in an enterprise context. 
