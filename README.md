# Developer Sandbox
Developing hybrid cloud services can be challenging because replicating backing services that reside outside the dynamic cloud stack is not always possible. In addition, dependencies between network and service configurations can become quite complex and often lead to extensive testing and the delay of a go-lives. This repository aims to accelerate service developments with a configurable sandbox for hybrid cloud environments. The development environment combines packaged applications, as binaries and/or containers or operating system extensions with backing services, to replicate a service composition as close as possible. The sandbox is built on NixOS and utilizes the Nix package manager to ensure robust reproducibility. However, with a few tweaks it can also run on [Windows (via WSL)](https://learn.microsoft.com/en-us/windows/wsl/about), [ChromeOS (via CROSH)](https://chromeos.dev/en/linux), and [macOS](https://github.com/LnL7/nix-darwin).
