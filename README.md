...
The operator sandbox provides an development and execution environment for operation engineers, migrating applications to a hybrid cloud. It introduces an alternative approach towards infrastructure automation that is determined by open specification and uses a normal programming language instead of propriatary tools. Today, many cloud engineers employ a combination of applications like Terraform, Ansible and Kubernetes to automate topology blueprints, system configurations, and container deployments. Yet provisioning tools, configuration management systems and container platforms overlap significantly in functionality and the scope of automations is rather limited. Designed for deployments, these tools integrate with orchestrators to automate resource provisioning from a shared infrastructure pool. Physical infrastructure and process automations that centralize control functions or enable ongoing management, are out of scope. Just the opposite, the approach simplifies cloud-native application management but introduces complexity for operators handling a mix of third-party applications with different requirements. The tools require engineers to combine system definitions and execution instructions in a common code base. This leads to fragmentated of design decisions and the lack of control makes it difficult to address technical, commercial, or regulatory requirements with appropriate management procedures. The sandbox enables developers run system configurations in an isolated environment and pull system definitions from an external source, regardless whether these represent orchestration tools or physical resources. Decoupling service definitions from the deployment allows to test  and confirm regulatory compliance, security policies, and infrastructure pricing before launching a service.

## Host System

Instead of utilizing virtual artifacts, the opertor sandbox promotes the use of declarative package managers like [Guix](https://guix.gnu.org/) or [Nix](https://github.com/NixOS/nix) to provide programmability on operating system level. Operators define a desired system state in configuration files that isolate the dependencies for software packages and ensure clean and reproducible systems without wrapping application runtimes into virtual machines or container. A functional programming language defines and instantiates purpose build systems with executable templates. The foundation is a strip down version of the linux operating system that only covers the most essential components communicating with the hardware. A sort of watchdog triggers loads additional packages with templates that match the platform requirements of an application without depending on an orchestrator, additional packaging or specific communication patterns. This enables operation teams to centralize system designs without owning the deployment and to track and to roll back system configurations in a similar way like immutable artifacts without abstraction of the runtime environment, network- and storage interfaces.
