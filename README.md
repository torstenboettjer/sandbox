...
The operator sandbox provides an development and execution environment for operation engineers, migrating applications to a hybrid cloud. It introduces an alternative approach towards infrastructure automation that is determined by open specification instead of propriatary tools in order to meet enterprise requirements. Today, many cloud engineers employ a combination of applications like Terraform, Ansible and Kubernetes to automate topology blueprints, system configurations, and container deployments. Yet provisioning tools, configuration management systems and container platforms overlap significantly in functionality and the scope for automations remains small. Designed for deployments, these tools integrate with orchestrators to automate resource provisioning from a shared infrastructure pool, rather than enabling ongoing management and centralizing control functions. This approach simplifies cloud-native application management but introduces complexity for operators handling a mix of third-party applications with different requirements. Deployment tools combine system definitions and execution instructions in a common code base, which leads to a fragmentation of design decisions. The lack of control makes it difficult to meet technical, commercial, or regulatory requirements with appropriate operation procedures and to ensure the integrity of business-critical data. The sandbox provides a runtime with API access in a safe environment and enables engineers to to rely on an API for topology and system definitions and to run isolated deployent instructions. This ensures security and control, and allows to connect control interfaces, regardless whether these represent orchestration tools or physical resources. Service launch and service definition remains decoupled, to confirm regulatory compliance, security policies, and justification for service decisions before a deployment.
