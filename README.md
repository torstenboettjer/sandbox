...
Developing hybrid cloud services can be challenging, because developers can not rely on an Internal Developer Platform (IDP) to provide all components in the target production format. Often, this leads to endless regression testing and pushes out the go-live, because networking, service discovery, dependency management in production environments is substatilly different compared to development labs. This repository provides an opinionated and foundational configuration, designed to empower operation engineers in effortlessly setting up local developer sandboxes. While backing services that persist data remain in a hosted environment, it streamlines the development of hybrid cloud services by enabling clear resource compositions that combine critical applications that will be hosted on-premise as well, with containerized applications destined for cloud deployment. While the core sandbox environment is built using NixOS for robust and reproducible setups, its flexibility allows it to run seamlessly across various platforms. Developers can leverage this framework on [Windows (via WSL)](https://learn.microsoft.com/en-us/windows/wsl/about), [ChromeOS (via CROSH)](https://chromeos.dev/en/linux), and [macOS](https://github.com/LnL7/nix-darwin) by utilizing the Nix package manager.
