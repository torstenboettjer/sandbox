...
The operator sandbox addresses operation engineers with an environment for the design and optimization of systems that should be migrated to a hybrid cloud. Cloud engineers usually employ a combination of applications like Terraform, Ansible and Kubernetes to automate system configurations, topology designs and container deployments. But configuration management systems, infrastructure-as-code and container platforms overlap significantly in functionality and were designed with a similar purposes but a different platform-design in mind. The focus is on deployment automation and is not yielding significant benefits in service operation. E.g. the code interpreted by these tools does not separate system definitions and execution instructions, which works well, when applications follow cloud-native design patterns and the entire platform is operated by a single provider. Complexity rises, when the number and the diversity of applications increases, multiple service provider get involved or when only a subset of an entire application portfolio can be migrated for technical, commercial or regulatory reasons. In order for the organization to scale assigning specialists with focus on specific technologies, a common datamodel is required to integrate the toolchain and deliver on the promise of continuous integrations and deployments. 

## Target System

A application neutral delivery model should be based on system templates that trigger to changes match the topology and the lifecycle of an application without prescribing specific communication patterns, packaging mechanisms or orchestration services. The language employed for system configurations and provisioning scripts should not prevent engineers from managing dependencies on operating system level or orchestrating services on mutable hosts. While the development of cloud controllers is derived from the idea that every system can be virtualized, evolving technologies raise the question, whether this is the only way to deliver programmable infrastructure. A package manager that interprets declarative templates enables engineers to provide purpose build operating systems. And distributing configuratons via git allows to centralize management tasks, to track and roll back system configurations. The nix package manager enables engineers to compose operating systems declaratively, without abstracting the runtime environment, network and storage interfaces. The strip down version of an the operating system that includes only the most essential components for basic functionality is extended with two main components:
