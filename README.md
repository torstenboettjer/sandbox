...
The definition of the host system separated from the home configuration and stored in the [flake.nix](./flake.nix) file. This allows to run the same toolset across developers independent from from any preference for the one or other operating system or hardware platform. Relying on a separate configuration file for system and platform definitions eases continuous deployments together with the application code. Separating configuration files tool enables system engineers using the **[Direnv](https://direnv.net/)** tool to provide purpose build systems accross multiple projects. It is a nix shell extension that loads and unloads software and configurations automatically, moving in and out a directory. System configurations and their dependencies are isolated and can be shared through a version control system, which helps to overcome one of the main complexity driver for DevOps in an enterprise context. Fast iterations with divergent structures in application development and service operation often leads to massive workload for operators. While development teams are organized around solutions to focus on the delivery of business functionality, operation teams are organized around technologies with specialists managing systems. Hence, operators required for 24x7 operation have to join multiple SCRUM teams with little time left to fulfill their day to day tasks. A tool like direnv allows operations enigneers to create per-project environments and relieve system specialists. 


Flakes are still classified as experimental feature, a respective flag is appended to `/etc/nix/nix.conf`. Flakes are also the natural home for platform definitions, however while the configuration of system software in production should be owned by the respective competence center in operation, in the development phase engineers often need the freedom determine the configuration together wiht a selection of system software components. Therefore service configurations and platform components are defined in the devenv.nix file. 

Nix supports multiple concepts of separating environment definitions. E.g. the default [flake.nix](./flake.nix) file can be extended with nix packages and stored in a new package directory. However, devenv comes offers additional features to streamline the development process and comes an on devenv.nix. In most cases, this file serves the purpose, for more advanced requirements might require the definition of overlays or a a specific package.nix, which is not covered here.  
