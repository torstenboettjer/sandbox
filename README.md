# Developer Sandbox
Developing hybrid cloud services can be challenging because replicating productive services is not always possible. But mismatches in network and service configurations lead to extensive regression tests that delay go-lives. This repository aims to accelerate service developments with an opinionated NixOS setup that aims to empower platform enigneers with configurable sandbox environments. Development environments are build on predfined service compositions that remain flexibile to add applications and runtimes. Components can be hosted services, operting system process are packaged applications as binaries and containers. The sandbox is built on NixOS and utilizes the Nix package manager to ensure robust reproducibility. However, with a few tweaks it can also run on [Windows (via WSL)](https://learn.microsoft.com/en-us/windows/wsl/about), [ChromeOS (via CROSH)](https://chromeos.dev/en/linux), and [macOS](https://github.com/LnL7/nix-darwin).
