...
The operator sandbox addresses operation engineers with an environment for the design and optimization of systems that should be migrated to a hybrid cloud. It represents a different approach to adopt infrastructure as code in an enterprise environment. The benefits of programmable infrastructure are out of question, but the toolset that is commonly used today, yield limited benefits in service operation. Cloud engineers employ a combination of applications like Terraform, Ansible and Kubernetes to automate system configurations, topology designs and container deployments. Yet configuration management systems, infrastructure-as-code and container platforms serve a similar purpose and overlap significantly in functionality. The focus is on provisioning and deployment not on automating management processes. While this is applicable for cloud service provider developing cloud-native applications, it rises complexity for service operator managing large quantities of applications from different vendors and aiming to outsource mainly infrastructure operations to a third party, but retaining control over their data for technical, commercial or regulatory reasons. Code interpreters that do not separate system definitions and execution instructions, make it hard to ensure regulatory compliance and security as well as commercial justifications without a separation of duties between command and control. And scaling an operation shared services and subject matter experts to serve across design, implementation and operation of services remains diffcult to achieve. Delivering on the promise of continuous integrations and deployments for enterprise services requires an integrated toolchain with a common data source for system definitions and execution modules, rather than a single application. And separating engineering tasks along delivery milestones like operational readiness, fulfillment and assurance is easier with an isolated environment that is open to support adjusted toolsets. 
